...
# 1. Клиенты получают DNS = AdGuard на малинке, ВТОРЫМ — сам роутер.
...
#
#    Зачем второй резолвер (испытание сторожа 2026-08-15): когда малинка молчит,
#    роутер НЕ МОЖЕТ перехватить запросы, адресованные 192.168.88.2 — клиент и
#    малинка в одном мосту, такой трафик коммутируется на L2 и в IP-firewall не
#    попадает вовсе (см. выстраданное правило №9). Единственный, кто способен
#    уйти с мёртвого адреса, — сам клиент, и делает он это только если знает
#    запасной. Пока малинка жива, запросы к 88.1 всё равно заворачиваются на
#    AdGuard, так что контроль не слабеет; в аварии сторож заворот снимает, и
#    роутер отвечает сам.
...
:foreach n in=$lanNets do={
  /ip dhcp-server network set $n dns-server="$piAddr,$routerAddr"
