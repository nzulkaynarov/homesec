...
Скрипт ставит два dst-nat-перехвата и hairpin masquerade
(`hs: block page hairpin`) — без него ответ панели приходит клиенту
«с неожиданного источника» и страница не открывается (выстраданное
правило №3). Masquerade скрывает от панели IP клиента, поэтому на
перехваченный запрос панель отвечает редиректом на `HS_PANEL_LAN_URL`
(по умолчанию `http://192.168.88.2:8000`) — прямое соединение приходит
уже с настоящим IP и показывает персональную причину блокировки.
