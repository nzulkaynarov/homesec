...
- **Полный аудит кода и прода — `docs/09-audit-2026-07.md`** (102 находки:
  ...
  Ничего не исправлено — только зафиксировано. Начинать с блока «Рекомендованный
  порядок работ» в шапке того файла. Топ на сейчас: (1) битый селектор
  `find address=` в аварийном откате DNS (docs/05-operations.md:45 и
  enable-adguard.rsc:17,31) — в аварию НЕ сработает; (2) delete устройства
  роняет reconcile-тик (orphan device_macs); (3) двойной учёт квоты (окно 90с
  при тике 60с); (4) UX детских страниц (405 после «попросить время»,
  meta-refresh стирает ввод, «снять паузу» не снимает ручную блокировку).
...
**Шаг 1 — вскрытие (журнал предыдущего бута должен был пережить ребут,
journald на Debian 12 персистентный):**

- `journalctl -b -1 -p warning` и `journalctl -b -1 -k | grep -iE "oom|out of memory"` — искать OOM-killer;
- `df -h`; `du -sh /var/log/* /opt/AdGuardHome/data/*` — не забита ли SD
  (querylog AdGuard — главный подозреваемый по росту);
- `free -h`; `systemctl status homesec homesec-bot` — потребление после старта.

**Шаг 2 — согласованные меры (владелец одобрил направление «watchdog
вместо ежедневного ребута»; ежедневный cron-reboot отвергнут как
костыль, маскирующий причину):**

1. Аппаратный watchdog: `RuntimeWatchdogSec=15` в
   `/etc/systemd/system.conf` (bcm2835_wdt) — Pi сам перезагружается при
   зависании. Главная мера.
2. journald: `SystemMaxUse=64M` (логи не смогут забить диск).
3. AdGuard: обрезать retention query-лога (24 ч) и статистики (7 дней).
4. Юнитам homesec/homesec-bot: `MemoryMax=250M`, `Restart=on-failure` —
   OOM убивает один сервис, а не всю малинку.
5. zram-swap (`zram-tools`) — запас памяти без износа SD.
6. MikroTik netwatch на 192.168.88.2: при падении Pi слать алерт в
   Telegram через `/tool fetch` (бот на Pi о своей смерти не сообщит);
   опционально — тем же netwatch авто-выключать/включать dst-nat `hs-dns`
   (автоматизация правила №1: дом с интернетом, пока Pi лежит).
   Осторожно: netwatch-скрипт трогает firewall — согласовать с
   принципом «правила ставит только .rsc», логичнее оформить отдельным
   .rsc с парой скриптов up/down.
