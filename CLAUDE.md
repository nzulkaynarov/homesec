...
- Осталось ОПЦИОНАЛЬНО (на решение владельца): дружелюбное DNS-имя для
  закладки «Время» через AdGuard rewrite (напр. `vremya.dom → 192.168.88.2`,
  открывать `http://<имя>:8000/me`; /me — публичный путь, поэтому имя работает
  в обход intercept-редиректа). Без него закладка = `http://192.168.88.2:8000/me`.
